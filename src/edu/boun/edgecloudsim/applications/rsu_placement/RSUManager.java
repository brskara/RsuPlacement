...
public class RSUManager extends EdgeServerManager{
	...
	public double[] getAvgUtilization(){
		int vmSize = localDatacenters.size();
		double[] utilizationArr = new double[vmSize + 1];

		// for each datacenter...
		for(int i= 0; i<localDatacenters.size(); i++) {
			Host host = localDatacenters.get(i).getHostList().get(0);
			EdgeVM vm = SimManager.getInstance().getEdgeServerManager().getVmList(host.getId()).get(0);
			double vmUtilization = vm.getCloudletScheduler().getTotalUtilizationOfCpu(CloudSim.clock());
			utilizationArr[i+1] = vmUtilization;
		}
		utilizationArr[0] = DoubleStream.of(utilizationArr).sum() / (double)vmSize;
	...
	private Datacenter createDatacenter(int index, Element datacenterElement) throws Exception{
		...
		List<RSU> hostList = createHosts(datacenterElement);
		
		String name = "Datacenter_" + index;
	...
	private List<RSU> createHosts(Element datacenterElement){

		// Here are the steps needed to create a PowerDatacenter:
		// 1. We need to create a list to store one or more Machines
		List<RSU> hostList = new ArrayList<>();
		...
		for (int j = 0; j < hostNodeList.getLength(); j++) {
			...
			Coordinate coordinate = new Coordinate(y_pos, x_pos);
			RSU host = new RSU(
					hostIdCounter,
